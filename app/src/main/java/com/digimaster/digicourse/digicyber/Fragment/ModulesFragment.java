...
//        mySwipeCourse = view.findViewById(R.id.swiperefreshFragModule);
        ...
        try {
            ...
            course_id = pref.getString(PrefContract.cour_id,"");
...
//        );
    ...
    private void initItem() {
        ...
        mApiService.getCourseDetail("course/get_detail/"+course_id+"/"+user_id).enqueue(new Callback<ModuleResponse>() {
            @Override
            public void onResponse(@NonNull Call<ModuleResponse> call, @NonNull Response<ModuleResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    List<ModuleDetailItem> onsiteItemList = response.body().getModuleDetail();
                    //Toast.makeText(context, ""+androidItemList, Toast.LENGTH_SHORT).show();

                    //recyclerCurr.setAdapter(adapter_r);
                    moduleAdapter = new ModuleAdapter(getActivity(), onsiteItemList, genProductAdapterListener());
