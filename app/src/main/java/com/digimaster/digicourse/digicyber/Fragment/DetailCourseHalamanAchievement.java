...
import com.digimaster.digicourse.digicyber.Adapter.LibraryDetailCourseAdapter;
...
public class DetailCourseHalamanAchievement extends Fragment {
    String token, userid,courseid;
    AlertDialog loading;
    SecuredPreference pref;
    Context context;
    LibraryDetailCourseAdapter onlineAdapter;
    BaseApiService mApiService;
    RecyclerView rvWishlist;
    RecyclerView.LayoutManager layoutManager;
    List<CourseDetailItem> onlineItemList = new ArrayList<>();
    Button btnDate;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        onlineAdapter = new LibraryDetailCourseAdapter(getActivity(), onlineItemList, genProductAdapterListener());
...
//        );
    ...
    public void loadOnCourse(){
        ...
        mApiService.getCourseDetail2(courseid).enqueue(new Callback<CourseDetailResponse>() {

            @Override
            public void onResponse(@NonNull Call<CourseDetailResponse> call, @NonNull Response<CourseDetailResponse> response) {
                if (response.isSuccessful()) {

                    //Log.d("logggN",""+response.body());
                    List<CourseDetailItem> dataItemList = response.body().getCourseDetail();
                    //Toast.makeText(context, ""+androidItemList, Toast.LENGTH_SHORT).show();
                    onlineAdapter = new LibraryDetailCourseAdapter(getActivity(), dataItemList, genProductAdapterListener());
